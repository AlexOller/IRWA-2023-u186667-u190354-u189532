...
def _load_tweets_as_dataframe(json_data):
    data = pd.DataFrame.from_dict(json_data, orient='index').transpose()
...
def generate_tweet_summary(tweet_text, max_length=5):
    ...
    #we remove https and others
    filtered_words = [word for word in filtered_words if not word.startswith('https')]
    filtered_words = [word for word in filtered_words if not re.match(r'@[\w_]+', word)]


    #add words to summary
    summary = ' '.join(filtered_words[:max_length])

    return summary

def _row_to_doc_dict(row: pd.Series):
    _corpus[row['Id']] = Document(row['Id'], generate_tweet_summary(row['Tweet']), row['Tweet'], row['Date'], row['Likes'],
                                  row['Retweets'],
